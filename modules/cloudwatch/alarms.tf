...
# This tag is stable across destroy/redeploy cycles.
#
# count = var.create_alb_alarms ? 1 : 0 gates evaluation entirely.
# When false (default), Terraform never calls DescribeLoadBalancers.
# This means standup full apply, teardown full destroy, and PR plans
# on cold environments all succeed without the ALB being present.
...
# ── ALB-Level Alarms ───────────────────────────────────────────────────────────
# Gated on var.create_alb_alarms. When count = 0, data.aws_lb is also count = 0
# so no AWS API call is made. When count = 1, data.aws_lb[0].arn_suffix resolves
# to the live ALB discovered via the LBC tag.
# These are created by the apply-cloudwatch-alarms pipeline job which runs after
# validate-deployment confirms all ArgoCD apps are Synced and Healthy.
...
resource "aws_cloudwatch_metric_alarm" "alb_5xx_rate_high" {
  ...
  metric_query {
    ...
    metric {
      ...
      stat        = "Sum"
      dimensions = {
        LoadBalancer = data.aws_lb.proshop[0].arn_suffix
      }
    }
  }
